...
export default function SceneNewPage() {
  ...
  useEffect(() => {
    ...
    if (title) {
      setSceneTitle(decodeURIComponent(title));
    } else {
      // 根据模式生成默认标题
      const titleMap: Record<string, string> = {
        ai_practice: "AI对练场景",
        ai_exam: "AI对练+考试场景",
        fixed_practice: "固定对练场景",
        fixed_exam: "固定对练+考试场景",
      };
      setSceneTitle(titleMap[m || "ai_practice"] || "AI对练场景");
    }
    // 场景说明从参数读取
    const desc = params.get("desc") || "";
    if (desc) {
      setSceneDescInput(decodeURIComponent(desc));
    }
  ...
  async function handleEditAttachmentsSelected(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    for (const file of files) {
      ...
      try {
        ...
        setEditAttachments((prev) => prev.map((item) => (item.fileId === localKey ? { ...item, fileId: uploaded.id, name: uploaded.name || file.name, status: "done" } : item)));
      } catch (err) {
  ...
  async function handleTopAttachmentsSelected(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    for (const file of files) {
      ...
      try {
        ...
        formData.append("file", file);
        const uploaded = await apiFetch<{ id: string; name: string; parseStatus: string; parseError?: string }>("/knowledge/files", { method: "POST", body: formData });
        setTopAttachments((prev) => prev.map((item) => (item.fileId === localKey ? { ...item, fileId: uploaded.id, name: uploaded.name || file.name, status: "done" } : item)));
  ...
  async function handleSave(status: "disabled" | "published") {
    ...
    setError("");
    try {
      const name = sceneTitle || (aiIdentity.replace(/\s+/g, " ").slice(0, 18) + (aiIdentity.length > 18 ? "…" : ""));
      ...
      <div className="page-section sc-mod">
        ...
        <div className="scene-form">
          ...
          <div className="form-actions scene-form-bottom-actions">
            <button className="btn outline" type="button" onClick={() => navigateBackOr('/?section=scenes')} disabled={submitting}>取消</button>

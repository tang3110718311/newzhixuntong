...
- Exposes admin on `13000` and API on `14000`.
...
docker compose --env-file env.test -f docker-compose.test.yml up -d zxt-api zxt-admin
...
- `NEXT_PUBLIC_API_BASE_URL` is baked into the admin build.
